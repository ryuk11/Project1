...
class Xlsx2csv:
    ...
    def convert(self, outfile, sheetid=1,filename='test'):
        ...
        if sheetid > 0:
            ...
            for s in self.workbook.sheets:
                ...
                #if not self.py3:
                sheetname = str(sheetname.encode('utf-8')).replace("b'","").replace("'","")
                #sheetname = str(sheetname.decode('utf-8'))		
                of = outfile
                print('Convert Method')
                print(outfile,filename,sheetname)
                if isinstance(outfile, str):
                    of = os.path.join(outfile, filename.lower()+ '~' +sheetname.lower() + '.csv')
		    #of = os.path.join(outfile, filename+'~'+sheetname + '.csv')
                    ...
                    try:
                        ...
                        # if cell is general, be aggressive about stripping any trailing 0s, decimal points, etc.
                            ...
                            if self.floatformat:
                                ...
                                if '%' in format_str:
                                    L += 1
                                #self.data = ("%." + str(L) + "2f") % float(self.data)
                                self.data = ("%.2f") % float(self.data)
...
if __name__ == "__main__":
    ...
    try:
        if os.path.isdir(options.infile):
            ...
            xlsx2csv.convert(outfile, sheetid,filename)
